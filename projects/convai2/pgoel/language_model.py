...
from mymodel import MyRNNModel
...
class MyLanguageModelAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        MyLanguageModelAgent.dictionary_class().add_cmdline_args(argparser)
        return agent
    ...
    def get_target_loss(self, data, hidden, targets):
        ...
        for i in range(1, targets.size(1)):
            ...
            output, hidden = self.model(targets.select(1, i - 1).view(1, bsz), self.hidden)
            self.hidden = self.repackage_hidden(hidden)
            output_flat = output.view(-1, len(self.dict))
    ...
    def get_predictions(self, data):
        ...
        while total_done < bsz and i <= self.opt['truncate_pred']:
            if i == 0:
                ...
                output, hidden = self.model(Variable(word_idx.view(1, bsz)), hidden)
            hidden = self.repackage_hidden(hidden)
    ...
    def batch_act(self, observations):
        ...
        if any(['labels' in obs for obs in observations]):
            ...
            if self.is_training == False:
                self.hidden = self.model.init_hidden(self.batchsize)
            self.is_training = True
            data_list, targets_list, _c, _v, y_lens = self.vectorize(observations, self.opt['seq_len'],
                                                                     self.is_training)
        else:
            ...
            data_list, targets_list, labels, valid_inds, y_lens = self.vectorize(observations, self.opt['seq_len'],
                                                                                 self.is_training)
